...
class MessageService {
  ...
  Future<List<ConversationModel>> fetchConversations(String userId) async {
    ...
    // FIX: only direct (non-group) conversations should get an "other
    // member" profile attached. This used to run for every conversation,
    // including group/club chats - which meant a club's conversation list
    // entry would show some random member's name/avatar instead of the
    // club's own title, because ConversationModel.fromJson's `other?.fullName`
    // check in the UI took priority over `title`. Restricting this lookup
    // to direct conversations is what actually keeps club chats visually
    // separate from DMs.
    final directConvIds = memberships
        .where((m) => (m['conversations'] as Map<String, dynamic>?)?['is_group'] != true)
        .map((m) => m['conversation_id'] as String)
        .toList();
